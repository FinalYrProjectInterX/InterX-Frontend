  ...
  return (
    <div className={`flex flex-row items-center justify-between px-6 py-6 ${styles.navStyle}`}>
      ...
      <div className='flex flex-row items-center justify-between'>
        ...
        {user.value && <button id="btnSidebarToggler" type="button" onMouseMove={(event)=>{event.preventDefault();setdropdown(true)}} onMouseLeave={(event)=>{event.preventDefault();setdropdown(false)}} className="relative py-2 px-4 text-2xl flex flex-row items-center text-black hover:text-gray-500">
        ...
        {!user.value && <div className="mx-4 text-xl">
